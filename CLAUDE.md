...
3. **已知待办**:
   ...
   - **房间频道注册表**(随游戏状态机实现):`room.voiceChannels = Set`(主频道+子频道 `room_{id}_wolf`),`switchVoiceChannel` 切换时注册/注销;房间销毁/状态机 ended 按注册表全踢;语音清扫的 live 判断与注册表统一(当前清扫已按子频道前缀规则设计,见心得 13)
